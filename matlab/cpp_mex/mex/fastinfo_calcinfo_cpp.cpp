...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) override {
        if (inputs.size() != 4 || outputs.size() > 1) {
            fail("fastinfo_calcinfo_cpp:usage",
                "fastinfo_calcinfo_cpp(X, Xm, Y, Ym) expects four inputs.");
        }
        ...
        dispatch_integer_array(inputs[0], "X", [&](const auto& x) {
            ...
            dispatch_integer_array(inputs[2], "Y", [&](const auto& y) {
                ...
                if (x.getNumberOfElements() != y.getNumberOfElements()) {
                    fail("fastinfo_calcinfo_cpp:shape", "X and Y must contain the same number of samples.");
                }
                outputs[0] = factory_.createScalar(fastinfo::typed::calc_info(
                    raw_data(x), xm, raw_data(y), ym, x.getNumberOfElements()));
            });
        });
    }
};
