...
targets(end+1) = make_target(cfg, 'fastinfo_calcinfo_cpp', {'fastinfo_calcinfo_cpp.cpp'}, false);
targets(end+1) = make_target(cfg, 'fastinfo_calcinfomatched_cpp', {'fastinfo_calcinfomatched_cpp.cpp'}, true);
targets(end+1) = make_target(cfg, 'fastinfo_calccmi_cpp', {'fastinfo_calccmi_cpp.cpp'}, false);
targets(end+1) = make_target(cfg, 'fastinfo_calccondcmi_cpp', {'fastinfo_calccondcmi_cpp.cpp'}, false);
targets(end+1) = make_target(cfg, 'fastinfo_calccmi_slice_cpp', {'fastinfo_calccmi_slice_cpp.cpp'}, true);
targets(end+1) = make_target(cfg, 'fastinfo_calcinfoperm_cpp', {'fastinfo_calcinfoperm_cpp.cpp'}, true);
targets(end+1) = make_target(cfg, 'fastinfo_calcinfoperm_slice_cpp', {'fastinfo_calcinfoperm_slice_cpp.cpp'}, true);
targets(end+1) = make_target(cfg, 'fastinfo_calcinfo_slice_cpp', {'fastinfo_calcinfo_slice_cpp.cpp'}, true);
targets(end+1) = make_target(cfg, 'fastinfo_eqpop_cpp', {'fastinfo_eqpop_cpp.cpp', 'fastinfo_eqpop_kernels.cpp'}, false);
targets(end+1) = make_target(cfg, 'fastinfo_eqpop_sorted_cpp', {'fastinfo_eqpop_sorted_cpp.cpp', 'fastinfo_eqpop_kernels.cpp'}, false);
targets(end+1) = make_target(cfg, 'fastinfo_eqpop_slice_cpp', {'fastinfo_eqpop_slice_cpp.cpp', 'fastinfo_eqpop_kernels.cpp'}, true);
targets(end+1) = make_target(cfg, 'fastinfo_eqpop_sorted_slice_cpp', {'fastinfo_eqpop_sorted_slice_cpp.cpp', 'fastinfo_eqpop_kernels.cpp'}, true);
end
...
if strcmp(fileName, 'fastinfo_eqpop_kernels.cpp')
